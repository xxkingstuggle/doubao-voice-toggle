...
1. 如果当前不是豆包输入法：
   ...
   - 如果系统当前有媒体音频在播放，暂停播放，并记录“结束后需要恢复”。
   - 记录当前焦点输入框的原始内容。
...
媒体暂停/恢复使用 macOS 的系统媒体播放键，并且只在启动时检测到正在播放音频时才会恢复。它适合 Apple Music、Spotify、浏览器视频/音乐等正常接入系统媒体控制的播放源。
...
暂停/恢复音乐也依赖同一项辅助功能权限，因为它需要模拟系统媒体播放键。
